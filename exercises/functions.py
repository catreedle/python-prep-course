# Multiply
# Examine the function invocation below. Write the function multiply, such that it accepts two arguments
# and returns the product of its arguments. You may assume that the function arguments will always be numbers.
def multiply(a, b):
    return a * b
...
# Cite the Author
# Let's generalize the function from the previous exercise. Implement a function named cite that takes 
# two string arguments: the author of the quote and what they said. It then prints the quote, as shown below.
...
# argument by an exponent of 2 (also known as squaring the number or raising the number to the power of 2).
...
multiples_of_three # this will output anything because function call require parentheses ()

# Three-way Comparison
# Write a function that compares the length of two strings. It should return -1 if the first string is 
# shorter, 1 if the second string is shorter, and 0 if they're of equal length. For example:
...
# Use Python's control structures to create a function that takes an ISO 639-1 language code and returns
...
# Locale Part 1
# Write a function that extracts the language code from a locale. A locale is a combination of a 
# language code, a region, and usually also a character set, e.g en_US.UTF-8.
...
# Building on your solutions from the previous exercises, write a function local_greet that takes a locale
# as input, and returns a greeting. The locale lets us greet people from different countries appropriately, 
